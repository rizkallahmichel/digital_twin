"""Degree of Dehydrogenation (DoD) calculations."""
...
class DoDConfig(BaseInfluxExperienceConfig):
    """Configuration for Degree of Dehydrogenation (DoD) computations."""
...
@dataclass(frozen=True)
class DoDResult:
    """Snapshot of the derived Degree of Dehydrogenation outputs."""

    experience: str
    timestamp: datetime
    dod_ratio: float
    released_h2_moles: float
    max_h2_moles: float
...
@dataclass(frozen=True)
class DoDWriteTarget:
    """Describes where the DoD ratio should be stored in InfluxDB."""
...
class DoDCalculator(InfluxCalculatorBase):
    """Binds Influx access and DoD-specific calculations."""

    def __init__(self, config: DoDConfig) -> None:
        super().__init__(config)
        self.config = config

    def compute(self) -> DoDResult:
        ...
        if temperature_k <= 0:
            raise RuntimeError("Temperature must be above 0 K for DoD calculation.")
    ...
    def write_result(self, result: DoDResult, target: DoDWriteTarget) -> None:
        """Persist the DoD ratio back to InfluxDB."""

        point = (
            Point(target.measurement)
            .tag(self.config.tag_key, self.config.experience)
            .field(target.field, result.dod_ratio)
    ...
    def iter_experience(
        ...
        stop: datetime,
        step_seconds: float,
    ) -> Tuple[Iterable[DoDResult], Optional[datetime], Optional[datetime]]:
        if step_seconds <= 0:
            raise RuntimeError("Sampling interval must be greater than zero seconds.")

        hydrogen_volume = self._timeline_for_scalar(self.config.hydrogen_volume, start, stop, required=True)
        pressure = self._timeline_for_scalar(self.config.pressure, start, stop, required=True)
        temperature = self._timeline_for_scalar(self.config.temperature, start, stop, required=True)
        lohc_mass = self._timeline_for_scalar(self.config.lohc_mass, start, stop, required=True)

        lohc_volume_timeline: Optional[_ScalarTimeline] = None
        lohc_density_timeline: Optional[_ScalarTimeline] = None
        if self.config.lohc_volume is not None:
            lohc_volume_timeline = self._timeline_for_scalar(self.config.lohc_volume, start, stop, required=True)
        else:
            if self.config.lohc_density is None:
                raise RuntimeError("Provide LOHC volume or density so it can be derived from mass.")
            lohc_density_timeline = self._timeline_for_scalar(self.config.lohc_density, start, stop, required=True)
        ...
        def generator() -> Iterable[DoDResult]:
            ts = actual_start
            while ts <= actual_end:
    ...
    def _build_result_from_values(
        ...
        dod_ratio = released_h2_moles / max_h2_moles

        return DoDResult(
            experience=self.config.experience,
            timestamp=timestamp,
            dod_ratio=dod_ratio,
            released_h2_moles=released_h2_moles,
